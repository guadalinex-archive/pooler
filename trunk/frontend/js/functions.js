 ...
 * Podemos decir que es el coraz�n de la aplicaci�n, con todas las funciones
 * necesarias para una correcta ejecuci�n.
 * 
...
function loadTreeDists(){
	...
	//cargamos el �rbol de directorio de distribuciones
...
function loadGridDistsUser(){
	if(myGridDistsUser != null){
		//cerramos popup "a�adir nueva distribuci�n"
		if($('popup')) closePopup();
...
function printListDistNoSelected(oDists){
	if(myGridDistsUser != null){
		//desmarcamos las que ya est�n seleccionadas
...
function updateUser(){
	if(evalInputUser()){
		...
		//montamos la cad�na con los datos del usuario
